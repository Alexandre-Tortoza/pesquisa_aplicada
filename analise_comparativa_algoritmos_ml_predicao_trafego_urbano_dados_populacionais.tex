...
\subsection{Análise dos Modelos}

A avaliação abrangente dos cinco algoritmos de aprendizado de máquina permite estabelecer ranking sistemático baseado em critério primário (R², capacidade explicativa da variância) e critério secundário (MAE, erro absoluto médio, aplicado quando R² apresenta diferenças inferiores a 0,02). A Tabela \ref{tab:ranking_modelos} sintetiza o desempenho final de cada modelo conforme avaliado nas respectivas seções.
...
\begin{tabular}{lcccc}
\hline
\textbf{Ranking} & \textbf{Algoritmo} & \textbf{R²} & \textbf{MAE (m)} & \textbf{RMSE (m)} \\
\hline
1º & Random Forest & \textbf{0,62} & 760,2 & 1298,6 \\
2º & XGBoost & 0,60 & 844,7 & 1327,3 \\
3º & K-Nearest Neighbors & 0,56 & 749,3 & --- \\
4º & Multilayer Perceptron & 0,50 & 962,3 & 1490,8 \\
5º & Regressão Linear & 0,10 & 1379,7 & 1990,8 \\
\hline
\end{tabular}
\caption{Ranking comparativo dos modelos de aprendizado de máquina}
\label{tab:ranking_modelos}
\end{table}

O Random Forest destaca-se como o modelo mais eficaz na previsão de congestionamento urbano, com R² de 0,62 e MAE de 760,2 metros, evidenciando sua habilidade em capturar interações complexas entre variáveis demográficas, temporais e espaciais por meio da agregação de 100 árvores de decisão. Sua robustez frente a outliers e insensibilidade à escala das variáveis tornam sua aplicação prática mais simples, dispensando pré-processamento extensivo. A análise de importância de variáveis revelou a população total como principal fator (37%), seguida por via expressa (15%), mês (15%) e hora do dia (13%), oferecendo insights valiosos para gestores urbanos. O XGBoost, com R² de 0,60 e MAE de 844,7 metros, apresenta desempenho competitivo, com vantagens como controle refinado de hiperparâmetros, treinamento incremental e inferência rápida, sendo ideal para contextos operacionais que exigem agilidade. O K-Nearest Neighbors, embora baseado em paradigma distinto de aprendizado por instâncias, alcança R² de 0,56 e MAE de 749,3 metros, validando parcialmente a hipótese de similaridade local, mas sua exigência computacional limita o uso em tempo real. O Multilayer Perceptron, com R² de 0,50 e MAE de 962,3 metros, demonstra que redes neurais profundas podem modelar não linearidades, embora enfrentem desafios em datasets tabulares moderados e apresentem menor interpretabilidade. Já a Regressão Linear, com R² de apenas 0,10, serve como baseline metodológico, evidenciando a inadequação da linearidade para este problema e reforçando a superioridade das abordagens não lineares. A escolha entre Random Forest e XGBoost deve considerar o contexto: enquanto o primeiro oferece maior acurácia e interpretabilidade, o segundo se destaca pela eficiência e flexibilidade. Ambos superam significativamente os demais modelos e podem ser combinados em ensemble heterogêneo via stacking, estratégia promissora para elevar a capacidade preditiva além dos 62% alcançados isoladamente pelo Random Forest.
